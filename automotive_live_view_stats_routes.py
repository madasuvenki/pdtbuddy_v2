...
def _current_running_builds(
    ...
    """Look up current Axiom builds from axiom_job_summary.

    "Current" means Axiom jobs whose state is either Running or Aborted.  The
    aborted jobs are intentionally included because they are still related to
    the active/current build investigation and their job IDs must be included in
    the cross-check JQL generated by the Current Running Build tab.
    ...
    try:
        ...
        cur.execute(
            ...
            SELECT build_id, build_name, software_product, product_flavor, device_count,
                   chip_ids, job_id, state, started_at, submitted_at, hours
            FROM pdt_stats_dashboard.axiom_job_summary
            WHERE LOWER(COALESCE(state, '')) IN ('running', 'aborted') AND ({where_sql})
            ORDER BY CASE WHEN LOWER(COALESCE(state, '')) = 'running' THEN 0 ELSE 1 END,
                     submitted_at DESC
            LIMIT 2000
            """,
            tuple(params),
        ...
        for row in cur.fetchall() or []:
            ...
            item["state_counts"][state] = _safe_int(item["state_counts"].get(state)) + 1
            if state not in item["states"]:
                item["states"].append(state)
            is_running = state.lower() == "running"
            item["has_running"] = bool(item.get("has_running")) or is_running
            item["state"] = "Running" if item["has_running"] else item["states"][0]
            if is_running and row.get("started_at"):
                item["started_at"] = str(row.get("started_at") or "")[:19]
            jid = str(row.get("job_id") or "").strip()
            if jid and jid not in item["job_ids"]:
                item["job_ids"].append(jid)
        # Show only builds that currently have at least one Running job.
        # Their job_ids still include all related Running + Aborted jobs for
        # the same build and product_flavor, so report/JQL cross-checks remain
        # complete without listing abandoned/fully-aborted builds on the UI.
        rows = [item for item in grouped.values() if item.get("has_running")]
...
def _auto_jira_aliases() -> Dict[str, List[str]]:
    return {
        ...
        "scenario": ["scenario", "Scenario", "axiom_scenario", "axiom_job", "axiom_job_id", "job_id", "job_url"],
    }
...
def _auto_jql_quote(value: Any) -> str:
    return '"' + str(value or "").replace("\\", "\\\\").replace('"', '\\"') + '"'


def _auto_gen45_current_jql(build: str, job_ids: Optional[List[str]] = None) -> str:
    from config import JIRA_PDT_FILTER_ID

    build = _auto_build_label(build)
    job_ids = [str(j).strip() for j in (job_ids or []) if str(j).strip()]
    parts = [f"project = DROIDBUG AND filter = {JIRA_PDT_FILTER_ID}"]
    if build:
        parts.append(f"summary ~ {_auto_jql_quote(build)}")
    if job_ids:
        scenario = " OR ".join(
            f"Scenario ~ {_auto_jql_quote('https://axiom.qualcomm.com/#/reports/job/' + job_id)}"
...
def _auto_is_dup_category(value: Any) -> bool:
    """Return True if cr_category or cr_status indicates a duplicate CR."""
    text = re.sub(r"[^a-z0-9]+", "", str(value or "").lower())
    return text in {"dup", "duplicate", "duplicates", "cannotduplicate", "invaliddup"}


def _auto_resolve_parent_cr(cr: str, cr_index: Dict[str, Any]) -> str:
    """Given a duplicate CR number, find its parent/canonical CR from cr_index.

    The cr_index (from run_consolidated_report) maps every raw CR alias to its
    canonical entry. The canonical entry carries 'canonical_cr' pointing to the
    parent. We also check DuplicateChangeRequests / ParentId from the raw Orbit
    data stored in cr_index.
    """
    if not cr or not cr_index:
        return ""
    cr_key = cr if cr.upper().startswith("CR") else f"CR{re.sub(r'[^0-9]', '', cr)}"
    entry = cr_index.get(cr_key) or cr_index.get(cr) or {}
    # canonical_cr is set by fetch_consolidated_report when a dup is rewritten
    canonical = str(entry.get("canonical_cr") or entry.get("cr_number") or "").strip()
    if canonical and canonical != cr_key:
        return canonical
    return ""


def _auto_flatten_consolidated_report(report: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for cr_row in (report.get("hierarchical_report") or []):
        ...
        if not jiras:
            ...
            rows.append({
                ...
                "is_dup":         is_dup,
                "parent_cr":      parent_cr,
                "last_instance":  cr_row.get("last_seen") or "-",
                "crash_type":     _auto_crash_type(cr_row.get("cr_title")),
                **fields,
            })
            continue
        ...
        for jira in jiras:
            ...
            rows.append({
                ...
                "is_dup":         is_dup,
                "parent_cr":      parent_cr,
                "last_instance":  jira.get("created") or jira.get("updated") or "-",
                "crash_type":     _auto_crash_type(title),
                **fields,
            })

    if not rows:
        # Fallback: flat jiras list (no hierarchical_report)
        for jira in (report.get("jiras") or []):
            title = jira.get("summary") or jira.get("title") or "-"
            ...
            rows.append({
                ...
                "is_dup":        is_dup,
                "parent_cr":     parent_cr,
                "last_instance": jira.get("created") or "-",
                "crash_type":    _auto_crash_type(title),
                **fields,
            })
    return rows
...
def _auto_gen45_build_report_payload(target_name: str, sp: str, selected_build: str = "", crash_types: Optional[set] = None, job_ids: Optional[List[str]] = None, plat_key: str = "") -> Dict[str, Any]:
    ...
    run the consolidated JQL report. The configured DB tables are only used to
    identify the SP/product-flavor and the active Axiom job IDs; they are not
    used to count crashes for the selected build.
    """
    crash_types = crash_types or {"system", "ssr", "process", "open_jira"}
    job_ids = [str(j).strip() for j in (job_ids or []) if str(j).strip()]

    if selected_build:
        build = _auto_build_label(selected_build)
        jql = _auto_gen45_current_jql(build, job_ids)
        try:
            import sys as _sys
            ...
            detail_rows = [
                row for row in _auto_flatten_consolidated_report(raw_report)
                if str(row.get("crash_type") or "system").lower() in crash_types
            ]
            jira_count = len({str(r.get("jira") or "").strip() for r in detail_rows if str(r.get("jira") or "").strip() and r.get("jira") != "-"})
            cr_count = len({str(r.get("cr") or "").strip() for r in detail_rows if str(r.get("cr") or "").strip() and r.get("cr") != "-"})
            return {
                "ok": True,
                "success": True,
                "sp": str(sp or ""),
    ...
    for fq, source in ((jiras_table, "jiras"), (open_table, "open_jira")):
        ...
        try:
            ...
            rows = []
        for row in rows:
            build = _auto_build_label(row.get("build"))
            if not build:
                continue
            ct = _auto_crash_type(row.get("title"), source)
            if ct not in crash_types:
                continue
            item = grouped.setdefault(build, {"build_id": build, "total_crashes": 0, "system_count": 0, "ssr_count": 0, "process_count": 0, "open_jira_count": 0, "cr_count": 0, "_crs": set()})
            item["total_crashes"] += 1
            item[f"{ct}_count"] = _safe_int(item.get(f"{ct}_count")) + 1
            cr_key = _auto_cr_key(row.get("cr"))
            if cr_key:
                item["_crs"].add(cr_key)
    builds = []
...
# ---------------------------------------------------------------------------
# Saved JQL Tabs API for Automotive/Gen4.5 targets
# Delegates to the generic helpers in live_view_stats_routes so the
# centralized scheduler covers these targets automatically.
# ---------------------------------------------------------------------------

@automotive_live_view_stats_bp.route("/api/automotive_live_view_stats/<string:target_name>/saved_jql_tabs", methods=["GET"])
@login_required
def api_auto_saved_jql_tabs_list(target_name: str):
    from live_view_stats_routes import _sjql_domain, api_lvs_saved_jql_tabs_list
    # Delegate to the generic handler by temporarily setting request context
    ...
    for tab in list_tabs(target_name, domain):
        ...
        tabs.append(row)
    return _jsonify({"ok": True, "target": target_name, "domain": domain, "tabs": tabs})


@automotive_live_view_stats_bp.route("/api/automotive_live_view_stats/<string:target_name>/saved_jql_tabs", methods=["POST"])
@login_required
def api_auto_saved_jql_tabs_save(target_name: str):
    ...
    if not _is_admin_user():
        return _jsonify({"ok": False, "error": "Access denied"}), 403
    ...
    try:
        ...
        return _jsonify({"ok": True, "tab": tab, "tabs": list_tabs(target_name, domain)})
    except Exception as exc:
        return _jsonify({"ok": False, "error": str(exc)}), 400


@automotive_live_view_stats_bp.route("/api/automotive_live_view_stats/<string:target_name>/saved_jql_tabs/<tab_id>", methods=["DELETE"])
@login_required
def api_auto_saved_jql_tabs_delete(target_name: str, tab_id: str):
    from live_view_stats_routes import _is_admin_user, _sjql_domain
    ...
    return _jsonify({"ok": True, "deleted": bool(deleted), "tabs": list_tabs(target_name, domain)})


@automotive_live_view_stats_bp.route("/api/automotive_live_view_stats/<string:target_name>/saved_jql_tabs/<tab_id>/report", methods=["GET", "POST"])
@login_required
def api_auto_saved_jql_tab_report(target_name: str, tab_id: str):
