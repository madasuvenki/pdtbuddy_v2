...
_IOT_CORE_DECK_DIR = 'iot_core_deck'


def _iot_core_deck_dir(target_name: str) -> str:
    return os.path.join(_target_coredeck_dir(target_name), _IOT_CORE_DECK_DIR)


def _iot_core_deck_history_dir(target_name: str) -> str:
    return os.path.join(_iot_core_deck_dir(target_name), 'history')


def _iot_core_deck_history_path(target_name: str, history_id: str) -> str:
    history_id = os.path.basename(_safe_str(history_id))
    if history_id == 'latest':
        return os.path.join(_iot_core_deck_dir(target_name), 'latest.json')
    if not history_id.lower().endswith('.json'):
        raise ValueError('Invalid history id')
    return os.path.join(_iot_core_deck_history_dir(target_name), history_id)


def _iot_safe_num(value, default=0.0):
    try:
        if value in (None, ''):
            return default
...
def _iot_json_default(obj):
    if isinstance(obj, (datetime, date)):
        return obj.isoformat()
    return str(obj)


def _iot_get_display_name(target_name: str) -> str:
    try:
        return dc.get_display_name_for_target(target_name) or target_name
    except Exception:
        info = dc.get_target_info(target_name) or {}
        return info.get('display_name') or info.get('target_display') or target_name


def _iot_target_options() -> list:
    rows = []
    for row in _all_targets_for_ui() or []:
        bu = _safe_str(row.get('bu_key')).upper()
        if bu in {'IOT', 'IOT_SW', 'IOT_PDT'} or 'IOT' in bu:
            rows.append(row)
    return rows or [
        row for row in (_all_targets_for_ui() or [])
        if 'IOT' in _safe_str(row.get('target')).upper() or 'IOT' in _safe_str(row.get('display_name')).upper()
    ]


def _iot_load_mtbf_rows(target_name: str) -> list:
    try:
        from dashboard_routes import _load_mtbf_json_payload
        payload = _load_mtbf_json_payload(target_name, 'MTBF') or {}
...
def _iot_meta_key(row: dict) -> str:
    return _safe_str(row.get('meta_id') or row.get('meta') or row.get('build') or row.get('build_id') or row.get('build_full'))


def _iot_build_label(row: dict) -> str:
    return _safe_str(row.get('build_full') or row.get('build') or row.get('build_id') or row.get('meta_id') or row.get('meta'))


def _iot_meta_rows(target_name: str, limit: int = 80) -> list:
    rows = _iot_load_mtbf_rows(target_name)
    out = []
    seen = set()
    for idx, r in enumerate(rows):
        meta = _iot_meta_key(r)
        build = _iot_build_label(r)
        key = (meta or build).upper()
        if not key or key in seen:
            continue
        seen.add(key)
        hours = _iot_safe_num(r.get('hours') or r.get('total_hours'))
        crashes = int(_iot_safe_num(r.get('crashes') or r.get('total_crashes')))
        mtbf = _iot_safe_num(r.get('mtbf')) or (round(hours / crashes, 2) if hours and crashes else 0)
        out.append({
            'meta_id': meta or build,
            'build': build or meta,
            'build_full': build or meta,
            'sp': r.get('sp') or r.get('product_line') or r.get('software_product') or _iot_get_display_name(target_name),
            'week': r.get('week') or r.get('date') or r.get('first_submitted') or '',
            'hours': round(hours, 1) if hours else 0,
            'crashes': crashes,
            'mtbf': round(mtbf, 2) if mtbf else '',
            'source_index': idx,
        })
    out.sort(key=lambda x: str(x.get('week') or ''), reverse=True)
    return out[:limit]


def _iot_fetch_target_counts(target_name: str) -> dict:
    db = _db_core_summary(target_name, [], {}, {}) or {}
    counts = db.get('counts') or {}
    return {
        'total_jiras': int(counts.get('total_jiras') or 0),
        'open_jiras': int(counts.get('open_jiras') or 0),
        'total_crs': int(counts.get('total_crs') or 0),
        'unique_crs': int(counts.get('unique_crs') or 0),
        'open_analysis_crs': 0,
        'crm_builds': 0,
        'eng_builds': 0,
        'crm_crashes': 0,
...
def _iot_hwpdt_counts(target_name: str) -> dict:
    counts = {
        'internal_secure': 0,
        'internal_non_secure': 0,
        'external_secure': 0,
        'failed': 0,
        'under_debug': 0,
        'source': 'hwpdt_pages',
    ...
    try:
        ...
        data = dr._load_hwpdt_job_audit_data() or {}
        chip_lookup = data.get('chip_lookup') or {}
        info = dc.get_target_info(target_name) or {}
        sp_name = _safe_str(info.get('sp_name') or target_name)
        family_prefix = ''
        try:
            family_prefix = dr._hwpdt_sp_family_prefix(sp_name, target_name)
        except Exception:
            family_prefix = sp_name.split('.')[0].upper()
        matched = set()
        for chip_id, entries in (chip_lookup or {}).items():
            for entry in entries or []:
                sp = _safe_str((entry or {}).get('software_product')).upper()
                if family_prefix and (sp == family_prefix.upper() or sp.startswith(family_prefix.upper() + '.')):
                    matched.add(_safe_str(chip_id).upper())
                    break
                if sp_name and sp_name.upper() in sp:
                    matched.add(_safe_str(chip_id).upper())
                    break
        total = len([c for c in matched if c])
        counts['internal_secure'] = total
        counts['total'] = total
        counts['tested_total'] = total
        counts['generated_at'] = data.get('generated_at') or ''
    except Exception as exc:
        counts['error'] = str(exc)
    return counts


def _iot_fetch_open_cr_rows(target_name: str) -> list:
    schema = _safe_str(dc.get_schema_for_target(target_name)).strip('`')
    info = dc.get_target_info(target_name) or {}
    prefix = _safe_str(info.get('db_prefix') or target_name).lower()
    if not schema or not prefix:
        return []
    conn = dc.get_mysql_connection_db(bu_key=schema)
    if not conn:
    ...
    cur = conn.cursor(dictionary=True)
    try:
        table = f'{prefix}_unique_crs'
        if not _table_exists(cur, schema, table):
            return []
        tbl = _bt(schema, table)
        cols = _table_cols(cur, tbl)
        cr_col = _first_existing_ci(cols, ['mapped_cr', 'cr', 'cr_number', 'stability_ticket'])
        title_col = _first_existing_ci(cols, ['cr_title', 'jira_title', 'title', 'summary'])
        area_col = _first_existing_ci(cols, ['cr_area', 'area', 'subsystem'])
        status_col = _first_existing_ci(cols, ['cr_status', 'status', 'final_status'])
        age_col = _first_existing_ci(cols, ['cr_age', 'overall_age', 'age'])
        occ_col = _first_existing_ci(cols, ['cr_occurrence', 'overall_cr_occurrence', 'jira_count', 'cr_____current_month'])
        last_col = _first_existing_ci(cols, ['jira_date__last_instance', 'last_instance', 'jira_date', 'updated_date'])
        pdt_col = _first_existing_ci(cols, ['pdt_tag', 'pdt tag', 'tag'])
        if not cr_col:
            return []
        status_expr = _sql_norm_expr(status_col) if status_col else "''"
        wh = [f"NULLIF(TRIM(`{cr_col}`), '') IS NOT NULL"]
        if status_col:
            wh.append(f"{status_expr} IN ('open','analysis','inanalysis')")
        if age_col:
            wh.append(f"CAST(COALESCE(`{age_col}`,0) AS UNSIGNED) > 14")
        occ_expr = f"COALESCE(`{occ_col}`, 1)" if occ_col else "1"
        select_cols = [
            f"`{cr_col}` AS cr",
            f"`{title_col}` AS title" if title_col else "'' AS title",
            f"`{area_col}` AS area" if area_col else "'Unknown' AS area",
            f"`{status_col}` AS status" if status_col else "'' AS status",
            f"`{age_col}` AS age" if age_col else "0 AS age",
            f"{occ_expr} AS jira_count",
            f"`{last_col}` AS last_reported_date" if last_col else "'' AS last_reported_date",
            f"`{pdt_col}` AS pdt_tag" if pdt_col else "'' AS pdt_tag",
        ]
        cur.execute(
            f"SELECT {', '.join(select_cols)} FROM {tbl} WHERE {' AND '.join(wh)} "
            f"ORDER BY CAST(COALESCE(`{age_col}`,0) AS UNSIGNED) DESC, CAST({occ_expr} AS UNSIGNED) DESC LIMIT 30"
        )
        rows = []
        for r in cur.fetchall() or []:
            d = dict(r)
            d['last_reported_jira'] = ''
            rows.append(d)
        return rows
    except Exception:
        return []
...
def _iot_snapshot_payload(data: dict) -> dict:
    target = _safe_str(data.get('target'))
    if not target:
        raise ValueError('target is required')
    selected = data.get('selected_metas') if isinstance(data.get('selected_metas'), list) else []
    if not selected:
        selected = _iot_meta_rows(target, 5)
    date_text = _safe_str(data.get('date')) or date.today().isoformat()
    info = dc.get_target_info(target) or {}
    display = _iot_get_display_name(target)
    milestone_overrides = data.get('milestones') if isinstance(data.get('milestones'), dict) else {}
    es_date = _safe_str(milestone_overrides.get('ES') or milestone_overrides.get('es') or info.get('es_date'))
    fc_date = _safe_str(milestone_overrides.get('FC') or milestone_overrides.get('fc') or info.get('fc_date'))
    cs_date = _safe_str(milestone_overrides.get('CS') or milestone_overrides.get('cs') or info.get('cs_date') or info.get('cs1_date'))
    counts = _iot_fetch_target_counts(target)
    db = counts.pop('_db', {}) or {}
    hwpdt_counts = _iot_hwpdt_counts(target)
    if isinstance(data.get('hwpdt_counts'), dict):
        overrides = data.get('hwpdt_counts') or {}
        for key in ('internal_secure', 'internal_non_secure', 'external_secure', 'failed', 'under_debug'):
            if key in overrides and _safe_str(overrides.get(key)) != '':
                hwpdt_counts[key] = int(_iot_safe_num(overrides.get(key), 0))
    hwpdt_total = int(hwpdt_counts.get('internal_secure') or 0) + int(hwpdt_counts.get('internal_non_secure') or 0) + int(hwpdt_counts.get('external_secure') or 0)
    hwpdt_counts['total'] = hwpdt_total
    hwpdt_counts['tested_total'] = hwpdt_total
    mtbf_rows = []
    devices = 0
    crashes_total = 0
    hours_total = 0.0
    for row in selected:
        if not isinstance(row, dict):
            continue
        h = _iot_safe_num(row.get('hours'))
        c = int(_iot_safe_num(row.get('crashes')))
        m = _iot_safe_num(row.get('mtbf')) or (round(h / c, 2) if h and c else 0)
        devices += int(_iot_safe_num(row.get('device_count'), 0))
        crashes_total += c
        hours_total += h
        mtbf_rows.append({
            'sp': row.get('sp') or row.get('product_line') or display,
            'meta_id': _iot_meta_key(row),
            'build': _iot_build_label(row),
            'hours': round(h, 1) if h else 0,
            'crashes': c,
            'mtbf': round(m, 2) if m else '',
            'week': row.get('week') or row.get('date') or '',
        })
    custom_device_split = []
    if isinstance(data.get('device_split'), list):
        for row in data.get('device_split') or []:
            if not isinstance(row, dict):
                continue
            name = _safe_str(row.get('name'))
            y = int(_iot_safe_num(row.get('y'), 0))
            if name and y >= 0:
                custom_device_split.append({'name': name, 'y': y})
    if custom_device_split:
        devices = sum(int(r.get('y') or 0) for r in custom_device_split)
    if not devices:
        devices = max(len(mtbf_rows), 1) * 5
    counts.update({
        'devices': devices + hwpdt_total,
        'sw_devices': devices,
        'hw_devices': hwpdt_total,
        'crm_builds': len(mtbf_rows),
        'eng_builds': 0,
        'crm_crashes': crashes_total,
        'eng_crashes': 0,
    })
    open_rows = _iot_fetch_open_cr_rows(target)
    counts['open_analysis_crs'] = len(open_rows)
    area_counter = Counter(_safe_str(r.get('area')) or 'Unknown' for r in open_rows)
    age_bins = {'d0_7': 0, 'd7_14': 0, 'd14_21': 0, 'd21p': 0}
    area_age_counter = Counter()
    for r in open_rows:
        age = _safe_int_value(r.get('age'))
        if age <= 7:
            age_bins['d0_7'] += 1
        elif age <= 14:
            age_bins['d7_14'] += 1
            ...
            age_bins['d21p'] += 1
            area_age_counter[_safe_str(r.get('area')) or 'Unknown'] += 1
    trend_rows = _iot_meta_rows(target, 30)
    trend = []
    for r in reversed(trend_rows):
        mtbf = _iot_safe_num(r.get('mtbf')) or (_iot_safe_num(r.get('hours')) / _iot_safe_num(r.get('crashes')) if _iot_safe_num(r.get('hours')) and _iot_safe_num(r.get('crashes')) else 0)
        trend.append({'label': _safe_str(r.get('meta_id') or r.get('build'))[:24], 'mtbf': round(mtbf, 2) if mtbf else 0})
    return {
        'schema_version': 1,
        'kind': 'iot_core_deck_snapshot',
        'target': target,
        'display_name': display,
        'date': date_text,
        'generated_at': _now_str(),
        'generated_by': _username(),
        'program': {
            'target_label': info.get('sp_name') or info.get('display_name') or display,
            'chip': info.get('chip_name') or info.get('chipset') or target,
            'oem': data.get('notes') or ['Update OEM/program notes in the notes box.'],
            'es': _json_date(es_date),
            'fc': _json_date(fc_date),
            'cs': _json_date(cs_date),
        },
        'notes': data.get('notes') if isinstance(data.get('notes'), list) else [],
        'kpis': counts,
        'device_split': custom_device_split or [
            {'name': 'SW PDT', 'y': counts.get('sw_devices') or 0},
        ],
        'hwpdt_counts': hwpdt_counts,
        'open_jiras_by_area': [{'name': k, 'y': v} for k, v in area_counter.most_common(8)],
        'open_crs_by_age': age_bins,
        'open_crs_by_area': [{'name': k, 'y': v} for k, v in area_age_counter.most_common(8)],
        'open_cr_rows': open_rows,
        'mtbf_rows': mtbf_rows,
        'mtbf_trend': trend,
        'summary_counts_source': db.get('schema') or '',
    }


def _iot_save_snapshot(snapshot: dict) -> tuple[str, str]:
    target = _safe_str(snapshot.get('target'))
    if not target:
        raise ValueError('target is required')
    snapshot = dict(snapshot)
    snapshot['updated_at'] = _now_str()
    snapshot['updated_by'] = _username()
    os.makedirs(_iot_core_deck_history_dir(target), exist_ok=True)
    metas = [_safe_path_part(_safe_str(r.get('meta_id') or r.get('build'))) for r in snapshot.get('mtbf_rows') or [] if isinstance(r, dict)]
    meta_part = '_'.join([m for m in metas if m][:4]) or 'iot_core_deck'
    history_id = f"{_safe_path_part(snapshot.get('date') or date.today().isoformat())}_{meta_part}_{datetime.now().strftime('%H%M%S')}.json"
    hist_path = os.path.join(_iot_core_deck_history_dir(target), history_id)
    latest_path = os.path.join(_iot_core_deck_dir(target), 'latest.json')
    _write_json_file(hist_path, snapshot)
    _write_json_file(latest_path, snapshot)
    return history_id, hist_path


@core_deck_bp.route('/pdt/iot_core_deck')
@login_required
def iot_core_deck_page():
    if not _target_group_access():
        return render_template('coming_soon_template.html', title='IOT Core Deck', message='Access denied.'), 403
    selected_target = _safe_str(request.args.get('target'))
    embed = str(request.args.get('embed') or '').strip().lower() in {'1', 'true', 'yes', 'on'}
    return render_template(
        'iot_core_deck.html',
        targets=_iot_target_options(),
        selected_target=selected_target,
        embed=embed,
    )


@core_deck_bp.route('/api/iot_core_deck/metas')
@login_required
def api_iot_core_deck_metas():
    if not _target_group_access():
        return jsonify({'ok': False, 'error': 'Access denied'}), 403
    target = _safe_str(request.args.get('target'))
    if not target:
        return jsonify({'ok': False, 'error': 'target is required'}), 400
    return jsonify({'ok': True, 'target': target, 'metas': _iot_meta_rows(target, int(request.args.get('limit') or 80))})


@core_deck_bp.route('/api/iot_core_deck/hwpdt_counts')
@login_required
def api_iot_core_deck_hwpdt_counts():
    if not _target_group_access():
        return jsonify({'ok': False, 'error': 'Access denied'}), 403
    target = _safe_str(request.args.get('target'))
    if not target:
        return jsonify({'ok': False, 'error': 'target is required'}), 400
    return jsonify({'ok': True, 'target': target, 'hwpdt_counts': _iot_hwpdt_counts(target)})


@core_deck_bp.route('/api/iot_core_deck/snapshot', methods=['POST'])
@login_required
def api_iot_core_deck_snapshot():
    if not _target_group_access():
        return jsonify({'ok': False, 'error': 'Access denied'}), 403
    try:
        snapshot = _iot_snapshot_payload(request.get_json(force=True, silent=True) or {})
        return jsonify({'ok': True, 'snapshot': snapshot})
    except Exception as exc:
        return jsonify({'ok': False, 'error': str(exc)}), 400


@core_deck_bp.route('/api/iot_core_deck/save', methods=['POST'])
@login_required
def api_iot_core_deck_save():
    if not _target_group_access():
        return jsonify({'ok': False, 'error': 'Access denied'}), 403
    try:
        data = request.get_json(force=True, silent=True) or {}
        snapshot = data if data.get('kind') == 'iot_core_deck_snapshot' else _iot_snapshot_payload(data)
        history_id, path = _iot_save_snapshot(snapshot)
        return jsonify({'ok': True, 'snapshot': snapshot, 'history_id': history_id, 'path': path})
    except Exception as exc:
        return jsonify({'ok': False, 'error': str(exc)}), 400


@core_deck_bp.route('/api/iot_core_deck/history')
@login_required
def api_iot_core_deck_history():
    if not _target_group_access():
        return jsonify({'ok': False, 'error': 'Access denied'}), 403
    target = _safe_str(request.args.get('target'))
    if not target:
        return jsonify({'ok': False, 'error': 'target is required'}), 400
    rows = []
    folder = _iot_core_deck_history_dir(target)
    if os.path.isdir(folder):
        for fname in sorted(os.listdir(folder), reverse=True):
            ...
            rows.append({
                ...
                'metas': [r.get('meta_id') or r.get('build') for r in (snap.get('mtbf_rows') or []) if isinstance(r, dict)][:6],
                'updated_at': snap.get('updated_at') or snap.get('generated_at') or '',
                'updated_by': snap.get('updated_by') or snap.get('generated_by') or '',
            })
    return jsonify({'ok': True, 'target': target, 'history': rows[:80]})


@core_deck_bp.route('/api/iot_core_deck/history/load')
@login_required
def api_iot_core_deck_history_load():
    if not _target_group_access():
        return jsonify({'ok': False, 'error': 'Access denied'}), 403
    target = _safe_str(request.args.get('target'))
    history_id = _safe_str(request.args.get('history_id'))
    if not target or not history_id:
        return jsonify({'ok': False, 'error': 'target and history_id are required'}), 400
    try:
        path = _iot_core_deck_history_path(target, history_id)
        snap = _read_json_file(path, {})
        if not snap:
            return jsonify({'ok': False, 'error': 'Snapshot not found'}), 404
        return jsonify({'ok': True, 'target': target, 'history_id': history_id, 'snapshot': snap})
    except Exception as exc:
        return jsonify({'ok': False, 'error': str(exc)}), 400


def _iot_build_ppt(snapshot: dict):
    ...
    def box(x, y, w, h, fill='#ffffff', line='#b7cbea'):
        shp = slide.shapes.add_shape(1, Inches(x), Inches(y), Inches(w), Inches(h))
        shp.fill.solid(); shp.fill.fore_color.rgb = rgb(fill)
        shp.line.color.rgb = rgb(line)
        return shp

    def text(x, y, w, h, value, size=10, bold=False, color='#111827', align=PP_ALIGN.LEFT):
    ...
    p = snapshot.get('program') or {}
    sp_title = _safe_str(p.get('target_label') or p.get('sp_name') or snapshot.get('display_name') or snapshot.get('target'))

    # Slide 1: dashboard overview, aligned with the web UI preview.
    # Keep this as the first slide users see in the browser-downloaded PPT.
    box(0.15, 0.12, 13.0, 0.45, '#0f4a85', '#0f4a85')
    text(0.35, 0.2, 8, 0.25, f"{sp_title} PDT Status", 20, True, '#ffffff')
    text(10.2, 0.22, 2.5, 0.25, f"Date: {snapshot.get('date') or ''}", 10, True, '#ffffff', PP_ALIGN.RIGHT)

    k = snapshot.get('kpis') or {}
    kpis = [('Total JIRAs', k.get('total_jiras')), ('Open JIRAs', k.get('open_jiras')), ('Total CRs', k.get('total_crs')), ('Unique CRs', k.get('unique_crs'))]
    for i, (label, value) in enumerate(kpis):
        x = 0.45 + i * 3.15
        box(x, 0.75, 2.95, 0.55)
        text(x + .12, .86, 1.8, .16, label, 10, True, '#111827', PP_ALIGN.CENTER)
        text(x + .12, 1.08, 1.8, .18, value or 0, 16, True, '#000000', PP_ALIGN.CENTER)

    box(0.3, 1.55, 6.35, 1.35)
    text(0.38, 1.6, 2.2, .2, '1  Program Overview', 11, True, '#0f4a85')
    text(0.65, 2.05, 1.7, .45, p.get('target_label') or snapshot.get('target'), 11, True, '#111827', PP_ALIGN.CENTER)
    text(2.35, 1.98, 2.35, .55, 'OEM\n' + '\n'.join(p.get('oem') or ['-']), 9, False)
    text(4.9, 1.9, 1.4, .65, f"ES - {p.get('es') or '-'}\nFC - {p.get('fc') or '-'}\nCS - {p.get('cs') or '-'}", 9, True)

    box(0.3, 3.0, 6.35, 1.35)
    text(0.38, 3.05, 2.0, .2, '3  Key Updates', 11, True, '#0f4a85')
    text(0.7, 3.55, 1.7, .45, f"{k.get('devices') or 0} Devices Allocated\nSW PDT - {k.get('sw_devices') or 0}\nHW PDT - {k.get('hw_devices') or 0}", 9, True, '#169b52')
    text(2.8, 3.55, 1.7, .45, 'Key Notes\n' + '\n'.join((snapshot.get('notes') or ['No manual notes'])[:3]), 9, True, '#fb8c00')
    text(4.9, 3.55, 1.4, .45, f"Peak Capacity\nSW PDT - {k.get('sw_devices') or 0}\nHW PDT - {k.get('hw_devices') or 0}", 9, True, '#0b5dbc')

    hw = snapshot.get('hwpdt_counts') or {}
    hw_is = int(hw.get('internal_secure') or 0)
    hw_ins = int(hw.get('internal_non_secure') or 0)
    hw_ex = int(hw.get('external_secure') or 0)
    hw_failed = int(hw.get('failed') or 0)
    hw_debug = int(hw.get('under_debug') or 0)
    hw_total = hw_is + hw_ins + hw_ex

    box(0.3, 4.45, 6.35, 1.45)
    text(0.38, 4.5, 2.8, .2, '4  MTBF Status', 11, True, '#0f4a85')
    mtbf_rows = (snapshot.get('mtbf_rows') or [])[:4]
    y = 4.9
    text(0.55, y, 2.7, .2, 'Software Product Build', 8, True, '#ffffff')
    for idx, r in enumerate(mtbf_rows):
        yy = 5.1 + idx * .18
        text(0.55, yy, 3.0, .15, r.get('build') or r.get('meta_id'), 7)
        text(3.75, yy, .7, .15, r.get('hours'), 7, False, '#111827', PP_ALIGN.CENTER)
        text(4.55, yy, .7, .15, r.get('crashes'), 7, False, '#111827', PP_ALIGN.CENTER)
        text(5.35, yy, .7, .15, r.get('mtbf'), 7, True, '#e11d48', PP_ALIGN.CENTER)

    box(0.3, 6.0, 6.35, 1.0)
    text(0.38, 6.05, 2.9, .2, '6  MSM Screening Status (HW PDT)', 10, True, '#0f4a85')
    text(0.65, 6.32, 5.5, .18, 'No failure Parts so far in HW PDT Testing.' if not hw_failed else 'Failure parts require debug.', 8, True, '#166534', PP_ALIGN.CENTER)
    text(0.7, 6.62, 5.5, .18, f"Received/Tested: Internal Secure {hw_is} | Internal Non-Secure {hw_ins} | External Secure {hw_ex} | Total {hw_total}", 7, False, '#111827', PP_ALIGN.CENTER)
    text(0.7, 6.82, 5.5, .15, f"Failed: {hw_failed} | Under Debug: {hw_debug}", 7, False, '#111827', PP_ALIGN.CENTER)

    device_split_rows = [r for r in (snapshot.get('device_split') or []) if isinstance(r, dict)]
    sw_device_split_total = sum(int(_iot_safe_num((r or {}).get('y'), 0)) for r in device_split_rows)
    max_device_split = max([int(_iot_safe_num((r or {}).get('y'), 0)) for r in device_split_rows] + [1])
    axis_max = max(5, int(((max_device_split + 4) // 5) * 5))
    box(7.0, 1.55, 5.95, 1.75)
    text(7.08, 1.6, 2.6, .2, f"2  Device Split - {sw_device_split_total}", 11, True, '#0f4a85')
    chart_x = 8.85
    chart_y = 1.92
    chart_w = 3.15
    bar_h = .12
    ...
    label_x = 7.25
    label_w = 1.45
    for i, r in enumerate(device_split_rows[:5]):
        yrow = chart_y + i * row_gap
        name = _safe_str(r.get('name'))
        if len(name) > 18:
            name = name[:17] + '…'
        val = int(_iot_safe_num(r.get('y'), 0))
        bar_w = max(.06, min(chart_w, float(val) / max(float(axis_max), 1) * chart_w))
        text(label_x, yrow - .015, label_w, .17, name, 6.8, False, '#111827', PP_ALIGN.RIGHT)
        box(chart_x, yrow, bar_w, bar_h, '#0b5dbc', '#0b5dbc')
        text(min(chart_x + bar_w + .04, 12.32), yrow - .015, .28, .15, val, 6.8, True)
    axis_y = chart_y + min(len(device_split_rows[:5]), 5) * row_gap + .10
    for tick in range(0, axis_max + 1, 5):
        xpos = chart_x + (float(tick) / max(float(axis_max), 1) * chart_w)
        text(xpos - .08, axis_y, .22, .11, tick, 5.6, False, '#111827', PP_ALIGN.CENTER)
    text(chart_x + chart_w / 2 - .55, axis_y + .16, 1.1, .13, 'No. of Devices', 6, False, '#111827', PP_ALIGN.CENTER)

    box(7.0, 3.4, 5.95, 1.35)
    text(7.08, 3.45, 2.4, .2, '3  Open JIRAs by Area', 11, True, '#0f4a85')
    for i, r in enumerate((snapshot.get('open_jiras_by_area') or [])[:5]):
        text(7.45, 3.8 + i * .17, 2.2, .12, r.get('name'), 7, True)
        text(10.2, 3.8 + i * .17, .8, .12, r.get('y'), 7, True, '#e11d48', PP_ALIGN.CENTER)

    box(7.0, 4.95, 2.75, .95)
    text(7.08, 5.0, 2.1, .2, '4  Stability Stats', 11, True, '#0f4a85')
    text(7.25, 5.38, 2.1, .2, f"CRM Builds {k.get('crm_builds') or 0}   Eng Builds {k.get('eng_builds') or 0}\nCRM Crashes {k.get('crm_crashes') or 0}   Eng Crashes {k.get('eng_crashes') or 0}", 8, True)

    box(9.9, 4.95, 3.05, .95)
    a = snapshot.get('open_crs_by_age') or {}
    text(9.98, 5.0, 2.7, .2, f"5  Open/Analysis CRs : {k.get('open_analysis_crs') or 0}", 11, True, '#0f4a85')
    text(10.1, 5.38, 2.5, .18, f"0-7d: {a.get('d0_7') or 0}   7-14d: {a.get('d7_14') or 0}\n14-21d: {a.get('d14_21') or 0}   >21d: {a.get('d21p') or 0}", 8, True, '#92400e')

    # Slide 2: open CR table + MTBF trend data table
    ...
    text(0.35, 0.35, 4.2, .25, 'Open CRs By Age (> 2 weeks)', 14, True, '#ff0000')
    y = .82
    headers = ['#', 'CR ID', 'TITLE', 'AREA', 'JIRA\nHITS', 'AGE\n(DAYS)', 'STATUS', 'PDT\nTAG', 'LAST REPORTED\nDATE', 'LAST REPORTED\nJIRA']
    widths = [.28, .62, 6.95, .58, .48, .50, .55, .45, 1.05, .95]
    x0 = .25
    table_w = sum(widths)
    box(x0, y - .05, table_w, .42, '#0f4a85', '#0f4a85')
    for i, h in enumerate(headers):
        text(x0 + sum(widths[:i]), y, widths[i], .28, h, 5.4, True, '#ffffff', PP_ALIGN.CENTER)
    for ridx, r in enumerate((snapshot.get('open_cr_rows') or [])[:13]):
        yy = 1.18 + ridx * .27
        if ridx % 2 == 0:
            box(x0, yy - .03, table_w, .23, '#f8fbff', '#ffffff')
        vals = [
            ridx + 1,
            clip(r.get('cr'), 12),
            clip(r.get('title'), 118),
            clip(r.get('area'), 14),
            r.get('jira_count'),
            r.get('age'),
            clip(r.get('status'), 12),
            clip(r.get('pdt_tag') or '--', 8),
            clip(r.get('last_reported_date'), 26),
            clip(r.get('last_reported_jira'), 18),
        ]
        for i, v in enumerate(vals):
            text(x0 + sum(widths[:i]), yy, widths[i], .16, v, 4.7, False, '#111827', PP_ALIGN.CENTER if i != 2 else PP_ALIGN.LEFT)

    text(2.35, 5.45, 8.6, .25, f"{snapshot.get('display_name') or snapshot.get('target')} - MTBF Trend", 13, False, '#111827', PP_ALIGN.CENTER)
    trend = (snapshot.get('mtbf_trend') or [])[-14:]
    if trend:
        chart_x0 = .75
        chart_y_label = 6.02
        chart_w = 11.8
        step = chart_w / max(len(trend), 1)
        for i, r in enumerate(trend):
            x = chart_x0 + i * step
            text(x, chart_y_label, min(step, .82), .18, clip(r.get('label'), 14), 4.5, False, '#111827', PP_ALIGN.CENTER)
            text(x, chart_y_label + .32, min(step, .82), .16, r.get('mtbf'), 6.2, True, '#2b73b9', PP_ALIGN.CENTER)

    # Final slide
    slide3 = prs.slides.add_slide(blank)
    slide = slide3
    box(0, 0, 13.333, 7.5, '#ffffff', '#ffffff')
    box(0.15, 0.12, 13.0, 0.45, '#0f4a85', '#0f4a85')
    text(0.35, 0.2, 8, 0.25, f"{sp_title} PDT Status", 20, True, '#ffffff')
    text(0, 3.15, 13.333, .55, 'Thank You', 34, True, '#0f4a85', PP_ALIGN.CENTER)
    text(0, 3.78, 13.333, .28, 'PDT Buddy', 15, False, '#64748b', PP_ALIGN.CENTER)
...
@core_deck_bp.route('/api/iot_core_deck/download_ui_ppt', methods=['POST'])
@login_required
def api_iot_core_deck_download_ui_ppt():
    """Create a browser-downloaded PPT from exact UI-rendered PNG slides.

    The web page captures the current rendered dashboard as images, including
    Chart.js charts and current layout. This avoids maintaining a separate
    hand-drawn PPT layout that can drift from the UI.
    """
    if not _target_group_access():
        return jsonify({'ok': False, 'error': 'Access denied'}), 403
    try:
        ...
        images = data.get('slide_images') if isinstance(data.get('slide_images'), list) else []
        if not snap:
            return jsonify({'ok': False, 'error': 'snapshot is required'}), 400
        if not images:
            return jsonify({'ok': False, 'error': 'UI slide images are required'}), 400
        ...
        for img_data in images[:2]:
            img_text = _safe_str(img_data)
            if ',' in img_text:
                img_text = img_text.split(',', 1)[1]
            raw = base64.b64decode(img_text)
            slide = prs.slides.add_slide(blank)
            slide.shapes.add_picture(BytesIO(raw), 0, 0, width=prs.slide_width, height=prs.slide_height)

        # Reuse the final slide for every PPT download.
        slide = prs.slides.add_slide(blank)
        from pptx.dml.color import RGBColor
        from pptx.enum.text import PP_ALIGN
        ...
        def rgb(hex_color):
            h = str(hex_color or '#000000').lstrip('#')
            return RGBColor(int(h[0:2], 16), int(h[2:4], 16), int(h[4:6], 16))
        def box(x, y, w, h, fill='#ffffff', line='#ffffff'):
            shp = slide.shapes.add_shape(1, Inches(x), Inches(y), Inches(w), Inches(h))
            shp.fill.solid(); shp.fill.fore_color.rgb = rgb(fill)
            shp.line.color.rgb = rgb(line)
            return shp
        def text(x, y, w, h, value, size=10, bold=False, color='#111827', align=PP_ALIGN.LEFT):
            ...
            run.font.color.rgb = rgb(color)
            return tb
        p = snap.get('program') or {}
        sp_title = _safe_str(p.get('target_label') or p.get('sp_name') or snap.get('display_name') or snap.get('target'))
        box(0, 0, 13.333, 7.5, '#ffffff', '#ffffff')
        box(0.15, 0.12, 13.0, 0.45, '#0f4a85', '#0f4a85')
        text(0.35, 0.2, 8, 0.25, f"{sp_title} PDT Status", 20, True, '#ffffff')
        text(0, 3.15, 13.333, .55, 'Thank You', 34, True, '#0f4a85', PP_ALIGN.CENTER)
        ...
        prs.save(buf)
        buf.seek(0)
        filename = f"IOT_Core_Deck_{_safe_path_part(snap.get('target') or 'target')}_{_safe_path_part(snap.get('date') or date.today().isoformat())}.pptx"
...
@core_deck_bp.route('/api/iot_core_deck/download_ppt')
@login_required
def api_iot_core_deck_download_ppt():
    if not _target_group_access():
        return jsonify({'ok': False, 'error': 'Access denied'}), 403
    target = _safe_str(request.args.get('target'))
    history_id = _safe_str(request.args.get('history_id') or 'latest')
    if not target:
        return jsonify({'ok': False, 'error': 'target is required'}), 400
    try:
        path = _iot_core_deck_history_path(target, history_id)
        ...
        if not snap:
            return jsonify({'ok': False, 'error': 'Snapshot not found'}), 404
        buf = _iot_build_ppt(snap)
        filename = f"IOT_Core_Deck_{_safe_path_part(target)}_{_safe_path_part(snap.get('date') or date.today().isoformat())}.pptx"
        return send_file(buf, as_attachment=True, download_name=filename, mimetype='application/vnd.openxmlformats-officedocument.presentationml.presentation')
    except Exception as exc:
        return jsonify({'ok': False, 'error': str(exc)}), 500
